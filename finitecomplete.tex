...
It is easy to see how adaptive test cases can be transformed into their denotational representation and vice versa.
...
Then the following facts hold for all $q\in Q$, $A_i\in\Sigma_I$, $e\in\Sigma$, and 
$u_i \in (\Sigma_I\times\Sigma_O)^*$.
\begin{eqnarray}
(A_1/\bot).(A_2/\bot)\in L(q) & \Leftrightarrow  &(A_2/\bot).(A_1/\bot)\in L(q)
\label{eq:eq1}
\\
u_1.(A_1/\bot)\dots(A_k/\bot).u_2\in L(q) & \Leftrightarrow & u_1.u_2\in L(q)
\label{eq:eq2}
\\
...
\end{lemma}
\begin{proof}
Formulas (\ref{eq:eq1}) and (\ref{eq:eq2}) are a direct consequence of the fact that all
...
the CSP transition graph contains an outgoing transition from graph node $q$ which is labelled by $e$. Since $A_2$ is supposed to be a super-set of $A_1$, this property holds for $A_2$ as well.
\end{proof}
