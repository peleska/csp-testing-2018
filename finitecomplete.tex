...
have a self-transition for every non-empty refusal of $n$.
...
The first is that the output $y$ is in the input $x$, so it is not $\bot$,
...
target state is $n$ itself, and $(s,x)$ is a failure of $P$.
%
...
\begin{align*}
  (n,x,y,n')\in h\Leftrightarrow & (y\in x\wedge s.y\in \trc(P)\wedge n'=G(P)/s.y)\vee(y=\bot \wedge (s,x)\in {\text {\rm Fail}}(P)\wedge n'=n)\end{align*}
\end{lemma}
\begin{proof}
