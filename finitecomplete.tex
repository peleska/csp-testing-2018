...
\begin{argue}
  ...
  & definition of $L$
  \\ %
  \Leftrightarrow (s,R)\in \failure(P)& {\text {Lemmas~\ref{exte} and \ref{cont}}}
\end{argue}
\end{proof}
%
%------------------------------------------------------------------------------
...
only a single input in any given state. A test execution is represented by
the parallel composition of the test FSM with the SUT. So, the test case provides
a specific input to the SUT determined by the current state of
the test case. It accepts every output and transits either to a fail-state
FAIL, if the output is wrong according to the test objectives, or to the next
test state uniquely determined  by the processed input/output pair. Another
state PASS indicates that the test has been completed without failure. Both
FAIL and PASS are deadlock states.
